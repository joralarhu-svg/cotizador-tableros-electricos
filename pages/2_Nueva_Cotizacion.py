...
with st.form("formulario_cotizacion", clear_on_submit=False):
    ...
    corriente_motor = c3.number_input("Corriente nominal por bomba (A)", min_value=0.1, value=14.0, step=0.1)
    altitud_msnm = c4.number_input(
        "Altitud de operación (msnm)", min_value=0, value=0, step=100
    )
    st.caption(
        "Todas las bombas se consideran disponibles para operación. "
        "La alternancia será gestionada por el sistema de control."
    )
    c1, c2, c3, c4 = st.columns(4)
    tension = c1.selectbox("Tensión (V)", [220, 380, 440])
    fases = c2.selectbox("Fases", [3, 1])
